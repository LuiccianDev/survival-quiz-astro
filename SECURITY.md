...
pnpm tracks the trust level of published packages (signed provenance, verified publisher, etc.). With `no-downgrade`, if a new version has *less* trust evidence than its previous release, the install is blocked.
